...
async fn handle_incoming_connection(listener: &TcpListener, app_window: &Window<impl Runtime>) -> Result<(), NetworkingError> {
...
pub fn start_listening_server(app_window: Window<impl Runtime>) {
    tokio::spawn(async move {
        ...
        loop {
            let connection_result = handle_incoming_connection(&listener, &app_window).await;
...
pub async fn connect_to_server(addr: String) -> Result<bool, NetworkingError> {
